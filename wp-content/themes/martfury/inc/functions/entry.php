	...
	function martfury_product_video() {
		...
		if ( $video_image ) {
			...
			if ( $video_html ) {
				...
				$video_wrapper = sprintf( '<a href="%s"></a>', esc_attr( $vid_html ) );
				$video_html    = '<div data-thumb="' . esc_url( $video_thumb[0] ) . '" class="woocommerce-product-gallery__image mf-product-video">' . $video_wrapper . $vid_html . '</div>';
			}
		}
